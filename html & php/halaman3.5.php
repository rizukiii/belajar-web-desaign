<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Halaman 3.5 HTML & PHP : IF STATEMENT/PENGECEKAN</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-QWTKZyjpPEjISv5WaRU9OFeRpok6YctnYmDr5pNlyT2bRjXh0JMhjY6hW+ALEwIH" crossorigin="anonymous">
</head>

<body>
    <div class="container">
        <!-- ini pengecekan staement nilai jika ada dua variable   -->
        <?php
        $nilai = 70;
        $absenMasuk = 85;
// simbol && dibaca "dan" di sistem komputer
// >= dibaca lebih besar sama deangan
// <= dibaca lebih kecil sama dengan 
        if ($nilai >= 80 && $absenMasuk >= 80): ?>
            <div class="alert alert-success" role="alert">
                Anda lulus
            </div>
            <!-- penggantian {} adalah : versi mas fahmi-->
        <?php elseif ($nilai >= 70 && $absenMasuk >= 70): ?>
            <div class="alert alert-warning" role="alert">
                Anda kurang bagus nilai nya
            </div>
        <?php elseif ($nilai >= 60 && $absenMasuk >= 60): ?>
            <div class="alert alert-danger" role="alert">
                Anda belum lulus
            </div>
        <?php else: ?>
            <div class="alert alert-danger" role="alert">
                Anda belum lulus
            </div>
            <!-- wajib di kasih end if kalo versinya mas fahmi -->
        <?php endif; ?>




    </div>
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js" integrity="sha384-YvpcrYf0tY3lHB60NNkmXc5s9fDVZLESaAA55NDzOxhy9GkcIdslK1eN7N6jIeHz" crossorigin="anonymous"></script>
</body>

</html>